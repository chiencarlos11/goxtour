...
class MobileExecMenu extends Component {
  constructor(props) {
    super(props);
  ...
  getExecs() {
    let Execs = this.state.execsData.execs.map(exec => (
      <div className="mobile_menu_cell" key={exec.id}>
        <div className="mobile_execImageBox">
          <img
            className="mobile_execImage"
            src={this.getPortrait(exec.id)}
            alt=""
          />
        </div>
        <div className="mobile_execNameBox">
          <div className="mobile_execName">{exec.name}</div>
          <div className="mobile_execTitle">{exec.title}</div>
        </div>
      </div>
    ));
  ...
  render() {
    return <div className="mobile_container_menu">{this.getExecs()}</div>;
  }
}

export default MobileExecMenu;
